...
function initializeAtasPage() {
    ...
    // URL da planilha de Atas publicada como CSV
    const SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRmulznw_fw-smQtiuFRn30AX1Z54g9lvoqbrJWx6MIUr6wUKW7AHxV6gN6htsXh2JcKMCfcfwgRoMZ/pub?output=csv';

    // Função para carregar e processar dados da planilha
    async function loadAtasFromSheet() {
        try {
            ...
            const rows = PapaParse.parse(csvText, { header: true, skipEmptyLines: true }).data; // Usando PapaParse para parsear CSV
            console.log("Dados parseados:", rows); // Log para debug
            return rows;
        } catch (error) {
            console.error("Erro ao carregar dados da planilha de Atas:", error);
    ...
    function renderTableRows(atas) {
        ...
        atas.forEach(item => {
            const row = createAtaRow(
                item.ID, // Certifique-se que o nome da coluna no CSV é 'ID'
                item.Descricao, // Certifique-se que o nome da coluna no CSV é 'Descricao'
                item.Tema, // Certifique-se que o nome da coluna no CSV é 'Tema'
                item.Responsavel, // Certifique-se que o nome da coluna no CSV é 'Responsavel'
                item.Status, // Certifique-se que o nome da coluna no CSV é 'Status'
                item.DataAbertura, // Certifique-se que o nome da coluna no CSV é 'DataAbertura'
                item.DataFechamento // Certifique-se que o nome da coluna no CSV é 'DataFechamento'
            );
            ...
            if (editBtn) { // Verifique se o botão existe antes de adicionar o listener
                 editBtn.addEventListener('click', () => {
                    ...
                    alert("A edição deve ser feita manualmente no Google Sheets.");
    ...
    function createAtaRow(id, descricao, tema, responsavel, status, dataAbertura, dataFechamento) {
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">
                <button class="btn-edit px-2 py-1 rounded text-xs mr-1">Editar (GS)</button> <!-- Indicativo de edição no Google Sheets -->
