...
function initializeGanttPage() {
    ...
    function setupGanttListeners() {
        ...
        // URL do arquivo JSON de Planejamento no Google Drive (download direto)
        const JSON_URL = 'https://drive.google.com/uc?export=download&id=13oUK8N4_GZUP5exaM6T3gUJnwSqlmq0b';
        ...
        async function loadTasksFromJson() {
            try {
                ...
                return rows.map(task => ({
                    ...task,
                    Inicio: new Date(task.Inicio), // Certifique-se que o nome da propriedade no JSON é 'Inicio'
                    Termino: new Date(task.Termino), // Certifique-se que o nome da propriedade no JSON é 'Termino'
                    Duracao: parseInt(task.Duracao, 10) || 0, // Certifique-se que o nome da propriedade no JSON é 'Duracao'
                    Custo: parseFloat(task.Custo) || 0, // Certifique-se que o nome da propriedade no JSON é 'Custo'
                    AvancoReal: parseInt(task.AvancoReal, 10) || 0, // Certifique-se que o nome da propriedade no JSON é 'AvancoReal'
                    AvancoPlanejado: parseInt(task.AvancoPlanejado, 10) || 0 // Certifique-se que o nome da propriedade no JSON é 'AvancoPlanejado'
                }));
            } catch (error) {
        ...
        function renderTable(tasks) {
            ...
            tasks.forEach((task, index) => {
                ...
                row.innerHTML = `
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${task.ID}</td> <!-- Certifique-se que o nome da propriedade no JSON é 'ID' -->
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${task.NomeTarefa}</td> <!-- Certifique-se que o nome da propriedade no JSON é 'NomeTarefa' -->
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${task.Predecessora || 'N/A'}</td> <!-- Certifique-se que o nome da propriedade no JSON é 'Predecessora' -->
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <button class="edit-btn" onclick="alert('Edite na fonte de dados (JSON)')">✏️ (Fonte)</button> <!-- Indicativo de edição na fonte -->
                    </td>
                `;
        ...
        function drawGanttChart(tasks) {
            ...
            const dataRows = tasks.map(task => {
                // Ajuste o nome da propriedade conforme seu JSON
                const dependencyId = task.Predecessora; // Supondo que 'Predecessora' seja o ID ou vazio
                return [
                    task.ID, // Certifique-se que o nome da propriedade no JSON é 'ID'
                    task.NomeTarefa, // Certifique-se que o nome da propriedade no JSON é 'NomeTarefa'
                    task.Inicio,
                    task.Termino,
